function [p,stepok,u1,res,iter,Gu,Glam]=stepsize_control_continuation(p,u1,res,iter,Gu,Glam,~)
% stepsize_control_continuation: continuation stepsize control called from numerical_continuation.
%
% USAGE:
%  [p,stepok,u1,res,iter,Gu,Glam]=stepsize_control_continuation(p,u1,res,iter,Gu,Glam,dss)
% 
% * u1,...,dss are inputs and outputs at the same time 
% * Non-trivial outputs:
...
dlam=get_para_lambda(p,u1)-get_para_lambda(p);
stepok=1; 
ds=p.sol.ds; 

if(abs(ds)<=p.nc.dsmin) % minimal small step size to allowed minimum (indep. of res)
 if res<p.nc.tol;  p.sol.ds=sign(ds)*p.nc.dsmin; % residual OK 
  stepok=1;   
 end
 if stepok==-1
     return; 
